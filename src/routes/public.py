...
def get_users_short_names():
    ...
    db = get_db()
    users = db.execute('SELECT DISTINCT name, short_name FROM users WHERE short_name IS NOT NULL AND deleted_at IS NULL AND lapen_approved = TRUE ORDER BY short_name').fetchall()
