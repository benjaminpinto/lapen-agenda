...
def add_match_result():
    ...
    try:
        if schedule_id:
            ...
            # Normalize player names to short_name if they're registered users
            player1_name = db.execute('SELECT short_name FROM users WHERE LOWER(short_name) = LOWER(%s) OR LOWER(name) = LOWER(%s)', 
                                     (schedule['player1_name'], schedule['player1_name'])).fetchone()
            player2_name = db.execute('SELECT short_name FROM users WHERE LOWER(short_name) = LOWER(%s) OR LOWER(name) = LOWER(%s)', 
                                     (schedule['player2_name'], schedule['player2_name'])).fetchone()
            winner_normalized = db.execute('SELECT short_name FROM users WHERE LOWER(short_name) = LOWER(%s) OR LOWER(name) = LOWER(%s)', 
                                          (winner_name, winner_name)).fetchone()
            
            p1 = player1_name['short_name'] if player1_name else schedule['player1_name']
            p2 = player2_name['short_name'] if player2_name else schedule['player2_name']
            winner = winner_normalized['short_name'] if winner_normalized else winner_name

            db.execute('''
                INSERT INTO match_statistics (schedule_id, player1_name, player2_name, winner_name,
                    player1_sets, player2_sets, player1_games, player2_games, match_type, match_date, added_by, score)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (schedule_id, p1, p2, winner,
                  player1_sets, player2_sets, player1_games, player2_games, schedule['match_type'], schedule['date'], request.user_id, data.get('score')))
